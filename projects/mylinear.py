...
plt.scatter(X[:, 3], Y, 1) #scatter要求维数相同
plt.show()


def data_provider(data, label, batchsize):       #每次访问这个函数， 就能提供一批数据， 取一部分数据更新模型权重
    length = len(label)
...
idx = 3
plt.plot(X[:, idx].detach().numpy(), X[:, idx].detach().numpy()*w_0[idx].detach().numpy()+b_0.detach().numpy()) # 计算图上的tensor不能直接用于绘制图像
